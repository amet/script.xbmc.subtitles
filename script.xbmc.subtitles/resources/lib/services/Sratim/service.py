...
TV_SEARCH_RESULTS_PATTERN = u"עונה (?P<season_num>\d*), פרק (?P<episode_num>\d*).*?subtitles\.php\?mid=(?P<sid>\d*)"
SEARCH_RESULTS_PATTERN = "subtitles\.php\?mid=(?P<sid>\d*)"
...
def search_subtitles( file_original_path, title, tvshow, year, season, episode, set_temp, rar, lang1, lang2, lang3, stack ): #standard input
    ...
    if tvshow:
        ...
        # and episode match
        for foundSeason,foundEpisode,sid in subtitleIDs:
            if ((foundSeason == season) and (foundEpisode == episode)):
